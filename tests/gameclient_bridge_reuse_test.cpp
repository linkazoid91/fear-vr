...
bool ExecutableSiblingPath(
    const wchar_t* fileName, wchar_t (&path)[MAX_PATH]) {
...
HMODULE LoadedExecutableSibling(const wchar_t* fileName) {
    ...
    if (!ExecutableSiblingPath(fileName, path)) {
        return nullptr;
    }
...
int main() {
    ...
    wchar_t gameClientPath[MAX_PATH]{};
    HMODULE gameClient =
        ExecutableSiblingPath(
            L"modules\\GameClient.dll", gameClientPath)
        ? LoadLibraryExW(
              gameClientPath, nullptr, LOAD_WITH_ALTERED_SEARCH_PATH)
        : nullptr;
