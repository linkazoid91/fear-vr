...
The integration branch now also preserves wrapper identity through
`SetIndices`/`GetIndices`, serializes binding and reset bookkeeping, releases
the binding anchor safely during device teardown, fails closed when required
factory/device/swap-chain hooks are unavailable, and reuses the early
app-local bridge from the split GameClient module directory.

The Win32 build and all 21 isolated tests pass, including classic factory to
D3D9Ex conversion, `PresentEx`, additional-swap-chain `Present`, managed index
identity/reset/device-teardown checks, IAT fallback, and GameClient bridge
reuse. No FEAR or OpenXR run has been performed, so these results do not change
the fault boundary or conclusions below. The next Retail run should use the
correlated lifecycle events to answer the existing checkpoint rather than add
another reset workaround.
